import React, {
  useEffect, useRef, useMemo, useReducer,
} from 'react';
...
load.planes = await axios.get(process.env.PLANES_URI, config).then((res) => res.data);
...
function LOAD() {
  ...
  const countLoad = () => {
    const v = {};
    const type = load.oTyp.iRef.current.value;
    let dc;
    ...
    if (type === 'AT-3') {
      for (const ID of load.IDs) {
        if (['oQ', 'oM', 'oAgc', 'oAdc'].some((id) => id === ID)) load[ID].iRef.current.value = v[ID];
      }
    }
  };
  ...
  const handleConvertFuel = () => {
    const fuel = parseFloat(load.iFuel.iRef.current.value);
    const Qf = parseFloat(load.oQf.iRef.current.value);

    if (fuel !== '' && !Number.isNaN(fuel)) {
      const F__ = parseFloat(fuel);
      const Qf__ = parseFloat(Qf);
  ...
  const handleChangeFuel = () => {
    ...
    if (type === 'AT-3') fArm__ = -0.0257;
    if (type === 'C-152') fArm__ = '';

    if (state.weigth === 'lbs') load.oQf.iRef.current.value = fLoad.kgToLbs(load.iFuel.iRef.current.value * 0.721);

    else load.oQf.iRef.current.value = (load.iFuel.iRef.current.value * 0.721).toFixed(3);

    if (state.fuel === 'Gl.') {
      load.oQf.iRef.current.value = fLoad.galToL(load.iFuel.iRef.current.value * 0.721);
    }
  ...
  const handleChangeCrew = () => {
    ...
    if (type === 'AT-3') crArm__ = 0.6;
    if (type === 'C-152') crArm__ = '';

    if (state.weigth === 'lbs') load.oMcr.iRef.current.value = fLoad.kgToLbs(load.iQcr.iRef.current.value * crArm__);

    else load.oMcr.iRef.current.value = load.iQcr.iRef.current.value * crArm__;
  };
  ...
  const handleChangeLuggage = () => {
    ...
    if (state.weigth === 'lbs') load.oMlg.iRef.current.value = fLoad.kgToLbs(load.iQlg.iRef.current.value * lgArm__);

    else load.oMlg.iRef.current.value = load.iQlg.iRef.current.value * lgArm__;
  };
  ...
  useEffect(() => {
    ...
    if (planeState) {
      for (const ID of load.IDs) {
        if (['i', 'o'].some((k) => k === ID.charAt(0))) load[ID].iRef.current.value = ' ';
      }

      if (load.planes.find((plane) => (plane.sign === planeState)).oQmax.includes('lbs') && state.weigth !== 'lbs') setButtons.toggleWeigth();

      if (load.planes.find((plane) => (plane.sign === planeState)).oQmax.includes('kg') && state.weigth !== 'kg') setButtons.toggleWeigth();

      handleConvertWeigth();

      for (const ID of load.IDs)// pobieranie wartości pól tablicy do texteditów
      {
        const param = load.planes.find((plane) => (plane.sign === planeState))[ID];
              ...
              return (
                ...
                >
                  <ButtonGroup
                    ...
                    sx={{
                      display: 'flex', justifyContent: 'right', gap: auxStyle.gap, '& button': { width: '100%' },
                    }}
                  >
